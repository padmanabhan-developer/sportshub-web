...
function SendFixtureInfo(PATH,cp,ppr,fixture_id,sport_id,date_from,date_end,search_text, rowid){
    ...
    success: function(data){
		...
		if(obj.total_records == obj.total_schedule) {
...
//alert(sport_id);
  ...
  function selectAllFixture(PATH) {
	var channel_id=document.getElementById("total_channel_ids").value;
	var alertmes; var status; 
	var check = document.getElementById("select_all").checked;
	//alert(check)
	if( check == false ){ 
		alertmes = 'Are you sure want to unschedule all channels ?';
		status = 'no';
	}
	else {
		alertmes = 'Are you sure want to schedule all channels ?';
		status = 'yes';
	}
	...
	if(confirm(alertmes))
   	{ 
		//alert(status)
		$.ajax({
			url: PATH+"establishment_schedule/add_schedule",
			type: "POST",
			data: {channel_id : channel_id, check:status},
			beforeSend: function(){
				   $('.preloader').show();
		   	},
			success: function(data){ 
