...
public function remove_gallery_image()
	...
	public function display_search_fixture()
	{
	 ...
	 public function show_admin_page1()
	{
		if ($this->session->userdata('admin_email')) 
		{
			//echo "test"; die;
		...
		$this->data['result'] = $this->Admin_model->paymenthistory();
		//echo "<pre>";
//		print_r($this->data['establishment_list']); die;

		//for pegination
		$this->data['total_records']=count($this->data['result']);
		$this->data['ppr']=20;
		$this->session->set_userdata('ppr',$this->data['ppr']);
		$cp=trim($this->input->get('cp'));
    	if(!empty($cp))
		...
		else
		{
			$this->data['cp']=1;}
    	$this->data['pagination']=$this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
		$this->data['result']=$this->Admin_model->paymenthistory("limit ".$this->data['pagination']['start_limit'].",".   $this->data['pagination']['end_limit']
);
		$this->load->view('admin/paymenthistory_display',$this->data);
	}
		else
		redirect('admin/login');
	}
 	...
 	public function show_admin_page()
	{
		...
		$this->data['search_text']=trim($this->input->get('search_text'));
		//$this->session->set_userdata('search_text',$this->data['search_text']);
        $this->data['subscibed']=trim($this->input->get('subscibed'));
        //echo $this->data['subscibed']; 
		$this->data['establishment_list']=$this->Admin_model->EstablishmentAccountList($this->data['date_from'],$this->data['date_end'],$this->data['search_text'],$this->data['subscibed']);
		//echo "<pre>";
//		print_r($this->data['establishment_list']); die;
		...
		else
		{
			$this->data['cp']=1;}
    	$this->data['pagination']=$this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
		$this->data['establishment_list']=$this->Admin_model->EstablishmentAccountList($this->data['date_from'],$this->data['date_end'],$this->data['search_text'],$this->data['subscibed'],"limit ".$this->data['pagination']['start_limit'].",".   $this->data['pagination']['end_limit']
);
...
$this->data['total_records'] = count($this->data['result']);
    	...
    	if(empty($this->data['cp'])){$this->data['cp']=$cp;}else{$this->data['cp']=$this->session->userdata('cp');}
    	$this->data['pagination'] = $this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
    	$this->data['result'] = $this->Admin_model->paymenthistory("limit ".$this->data['pagination']['start_limit'].",".
		   $this->data['pagination']['end_limit']);




			$this->load->view('admin/paymenthistory',$this->data); 
	...
	public function viewpayment(){
		...
		if($this->session->userdata('admin_email')) {
			...
			$this->data['result'] = $this->Admin_model->viewpaymentinfo($esid,$cusid);
			$this->load->view('admin/viewpayment',$this->data); 
	...
	public function invoice(){
		if($this->session->userdata('admin_email')) {
			 $this->load->model('Admin_model');
			$esid = $this->input->get('esid'); 
			$subsid = $this->input->get('subsid');
			//$esid = "902";
			//$subsid ="12";
			$result = $this->Admin_model->generateinvoice($esid,$subsid);
...
$pdf->WriteHTML($stylesheet3,1);
		...
		if( $row->subscription_plan == 'standard_monthly_plan') {
			$plan= 'Sportshub365: Monthly Subscription Invoice'; $sub = "Monthly";
		}elseif ($row->subscription_plan == 'standard_yearly_plan') {
			$plan= 'Sportshub365: Yearly Premium Subscription Invoice'; $sub = "Yearly";
		}
				...
				$this->Admin_model->updateinvoicestatus($esid,$subsid,$ino. ".pdf");
...
public function upgdwng_ajax()
	{
		$id = $this->input->post('id');
		$profile = $this->Admin_model->GetSubscriptionDetails($id);
		
		if($id){
			if($profile['customer_id']){

				Stripe::setApiKey('sk_live_7pYubIHXP3L4WiCWAtqbnf7r');
				$customer_stripe = Stripe_Customer::retrieve($profile['customer_id']);
				$customer_stripe->delete();
			}
			$status = $this->Admin_model->Upgradedonwgradeaccount($id);	
			return true;
		}
  }
...
public function value_required($value,$field)
	...
	public function channels()
	{
		...
		if(!empty($this->data['delete']))
		{
			$this->Admin_model->ChangeChannelStatus($this->data['delete_id']);
		...
		$caller=$this->input->post('caller'); 
		$this->data['caller']= $caller;
		$this->data['search_channel'] = '';
		$this->data['total_records'] = $this->Admin_model->ChannelListCount('');
		...
		$this->data['channel_list'] = $this->Admin_model->ChannelList('',"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);
		//print_r($this->data['channel_list']);
		
		$values = array();
		$this->data['attribute_search']=$this->Admin_model->SearchFormChannelAttribute($values);
		$this->data['attribute_newest']=$this->Admin_model->AddChannelAttribute($values);

			$this->load->view('admin/channels',$this->data);
		}
		else
			redirect('admin/login');
	}
 	public function show_admin_page_channel()
	{
		if ($this->session->userdata('admin_email')) 
		{
		$this->data['active_schedule']='on';
		$this->data['search_channel']=trim($this->input->get('search_channel'));
		//for pegination
		$this->data['total_records']=$this->Admin_model->ChannelListCount($this->data['search_channel']);
		$this->data['ppr']=20;
		//$this->session->set_userdata('ppr',$this->data['ppr']);
		$cp=trim($this->input->get('cp'));
    	if(!empty($cp))
		...
		else
		{
			$this->data['cp']=1;}
    	$this->data['pagination']=$this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
		$this->data['channel_list']=$this->Admin_model->ChannelList($this->data['search_channel'],"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']
);
		$this->load->view('admin/channel_display',$this->data);
	}
		else
		redirect('admin/login');
	}
	public function providers()
	{
		...
		if(!empty($this->data['delete']))
		{
			$this->Admin_model->ChangeProoviderStatus($this->data['delete_id']);
		...
		$caller=$this->input->post('caller'); 
		$this->data['caller']= $caller;
		$this->data['search_provider']='';
		$this->data['total_records'] = $this->Admin_model->ProviderListCount('');
		
		$this->data['ppr']=20;
		$cp=1;
    	if(empty($this->data['cp'])){$this->data['cp']=$cp;}else{$this->data['cp']=1;}
    	$this->data['pagination'] = $this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
		//print_r($this->data['pagination']);
		$this->data['provider_list'] = $this->Admin_model->ProviderList('',"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);
		//print_r($this->data['provider_list']);
		
		$values = array();
		$this->data['attribute_search']=$this->Admin_model->SearchFormProviderAttribute($values);
		$this->data['attribute_newest']=$this->Admin_model->AddProviderAttribute($values);

			$this->load->view('admin/providers',$this->data);
		}
		else
			redirect('admin/login');
	}
 	public function show_admin_page_provider()
	{
		if ($this->session->userdata('admin_email')) 
		{
		$this->data['active_schedule']='on';
		$this->data['search_provider']=trim($this->input->get('search_provider'));
		//for pegination
		 $this->data['total_records']=$this->Admin_model->ProviderListCount($this->data['search_provider']);
		$this->data['ppr']=20;
		//$this->session->set_userdata('ppr',$this->data['ppr']);
		$cp=trim($this->input->get('cp'));
    	if(!empty($cp))
		...
		else
		{
			$this->data['cp']=1;}
    	$this->data['pagination']=$this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
		$this->data['provider_list']=$this->Admin_model->ProviderList($this->data['search_provider'],"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);
		$this->load->view('admin/provider_display',$this->data);
	}
		else
		redirect('admin/login');
	}
	public function provider_edit() {
	  ...
	  if($this->session->userdata('admin_email')) {
	   ...
	   $values=array(); 
	   $this->data['msg']="";
	   $provider_id = $this->input->get('id');
	   $this->data['provider_id'] = $provider_id;
			if ( $provider_id ) 
   			{
				 
		$this->data['search_text_provider']='';
		$this->data['total_records'] = $this->Admin_model->ChannelProviderListCount('',$provider_id);
		$this->data['ppr']=20;
		$cp=1;
    	if(empty($this->data['cp'])){$this->data['cp']=$cp;}else{$this->data['cp']=1;}
    	$this->data['pagination'] = $this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);

		$this->data['provider'] = $this->Admin_model->GetProviderDetail($provider_id);
		$this->data['channel_list']=$this->Admin_model->ChannelProviderList('',$provider_id,"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);
		$provider = $this->data['provider'];
		//echo "<pre>";
		//print_r($values_rating); die;
		$values = array();
		$this->data['attribute_search']=$this->Admin_model->SearchFormProviderChannelAttribute($values);
		$this->data['attribute_newest']=$this->Admin_model->AddProviderAttribute($values);
				
				if($caller == "Provider")
				{
					$values['provider']=trim($this->input->post('provider'));
					$values['comment']=trim($this->input->post('comment'));
					$this->load->library('form_validation'); 
				   
					$arrData= array('provider' => $values['rating'],
									'comment' => $values['comment']);
									   
					$this->Admin_model->UpdateRatingInfo($arrData, $rating_ref);

					redirect('admin/provider_edit/'.$rating_ref.'');
				}
				
				$this->load->view('admin/provider_edit', $this->data );
			}
			else { 
				redirect('admin/login');
			}	
 	...
 	public function show_admin_page_provider_channel()
	{
		if ($this->session->userdata('admin_email')) 
		{
			$this->data['active_schedule']='on';
			$this->data['search_text_provider']=trim($this->input->get('search_text_provider'));
			//for pegination
		   $provider_id = $this->input->get('provider_id');
		   $this->data['provider_id'] = $provider_id;

			$this->data['total_records']=$this->Admin_model->ChannelProviderListCount($this->data['search_text_provider'],$provider_id);
			$this->data['ppr']=20;
			//$this->session->set_userdata('ppr',$this->data['ppr']);
			$cp=trim($this->input->get('cp'));
			if(!empty($cp))
			...
			else
			{
				$this->data['cp']=1;}
				$this->data['pagination']=$this->Pagenation($this->data['cp'], $this->data['ppr'],$this->data['total_records']);
				$this->data['channel_list']=$this->Admin_model->ChannelProviderList($this->data['search_text_provider'],$provider_id,"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);
			$this->load->view('admin/provider_edit_display',$this->data);
		}
		else
		redirect('admin/login');
	}
	function update_channel_display_name(){
		if ($this->session->userdata('admin_email')) 
		{
		   $chid = $this->input->post('id');
		   $type = $this->input->post('type');
		   $ch_dp_name = $this->input->post('ch_dp_name');
		   if($chid && $type='update_ch_dp_name'){
			  $status =  $this->Admin_model->UpdateChennelDisplayName($chid,$ch_dp_name );
			  echo $status;
		   }
		}
	}
	function update_provider_channel_info(){
		if ($this->session->userdata('admin_email')) 
		{
		   ...
		   $mode = $this->input->post('mode');
			  ...
			  $status =  $this->Admin_model->UpdateProviderChennelInfo($provider,$channel, $mode);
			  echo $status;
		}
	}
	...
	function create_new_provider(){
		if ($this->session->userdata('admin_email')) 
		{
		   ...
		   if($prov_name){
			  $status =  $this->Admin_model->CreateNewProvider($prov_name);
			  echo $status;
		   }
		}
	}
	function update_provider_name(){
		if ($this->session->userdata('admin_email')) 
		{
		   $pid = $this->input->post('id');
		   $type = $this->input->post('type');
		   $p_name = $this->input->post('provider_name');
		   if($pid && $type='update_p_name'){
			  $status =  $this->Admin_model->UpdateProviderName($pid,$p_name );
			  echo $status;
		   }
		}
	}
   public function delete_provider()
	{
		...
		if($caller == "delete"){
				$rowdeleId = $this->input->post('deleteid');
				if($rowdeleId){
					$rowdelete = $this->Admin_model->DeleteProviderInfo($rowdeleId);	
					return true;
					}
			}
			else echo 0;
