...
public function login_facebook(){
  ...
  public function login()
  {
	   ...
	   if($caller == "Send")
	   {
		    ...
		    {  
			 $msg=$this->App_model->CheckUser($values['email'],myencrypt($values['password']));

			 if($msg=='success')
			 {
			 	$user_id=$this->App_model->GetUserId($values['email']);
		        $user_ref_id=$user_id[0]->user_id;
				//$est_info_id = $this->Establishment_model->GetEstInfoId($est_ref_id);
				$newdata=array('app_email'=>$values['email'],
			 		'password'=>myencrypt($values['password']), 'user_ref_id'=>$user_ref_id);
		  ...
		  // Automatically picks appId and secret from config
			...
			$this->data['attribute']=$this->App_model->LoginFormAttribute($values);
			        // OR
	   ...
	   if($caller == "Send")
	   {
		    ...
		    {  
			 if($values['password']==$values['re_password'])
			 {
				...
				$checkuser = $this->App_model->checkExstingUser($values['email']);
				if($checkuser == FALSE){
				$arrData= array(
			    ...
			    $this->App_model->RegisterSubscription($arrData);
...
<strong>Email :</strong> '.$values['email'].'<br/><strong>Password :</strong> '.$values['password'].'</p>';
				...
				$lastUser = $this->App_model->gerLastuser($values['email']);
		       // $est_ref_id=$user_id[0]->id;
				$user_ref_id = $user_id[0]->user_id;
				$_SESSION['user_ref_id'] = $user_ref_id;
			     	redirect('app/login');
 					$this->data['email']=$values['email'];
					$this->data['password']=$values['password'];
		   ...
		   $this->data['attribute']=$this->App_model->SignupFormAttribute($values);
		   $this->load->view('app/signup',$this->data);
	 }
	...
	public function forgot(){
	   ...
	   if($caller == "Send")
	   {
		    ...
		    { 
			 ...
			 $msg=$this->App_model->ResetUser($values['email'],$randompwd,$pwdencrypt );
			...
			$this->data['attribute']=$this->App_model->ForgotFormAttribute($values);
		}
		   $this->load->view('app/forgot',$this->data);
