...
class App_my_tv_schedule extends MY_Controller {
	...
	//	print_r($ar);
		...
		$this->data['fixture_list_count']=$this->App_tv_model->AllFixture($user_ref,$this->data['sport_id'],$this->data['date_from'],$this->data['date_end'],$this->data['search_text']);
		...
		$this->data['attribute_search']=$this->App_tv_model->SearchFormAttribute($values);
		...
		$this->data['fixture_list']=$this->App_tv_model->AllFixture($user_ref,$this->data['sport_id'],$this->data['date_from'],$this->data['date_end'],$this->data['search_text'],"limit ".$this->data['pagination']['start_limit'].",".$this->data['pagination']['end_limit']);

		//print_r($this->data['fixture_list']);

		$this->load->view('app/my-tv-schedule',$this->data);
		}
		else
			redirect('app/login');
 ...
 public function display_search_my_tv_fixture()
	{
		...
		$this->data['fixture_list']=$this->App_tv_model->AllFixture($user_ref,$this->data['sport_id'],
		...
		$this->load->view('app/display-my-tv-schedule',$this->data);
	}
		else
		redirect('app/login');
	...
	public function make_fixture_schedule()
	{
		...
		$this->App_tv_model->MakeFixtureSchedule($this->data['fixture_id'],$user_ref);

		$this->data['fixture_list']=$this->App_tv_model->AllFixture($user_ref,
	...
	{ 
		...
		//	print_r($ar);
			...
			$this->data['active_schedule']='on';
			$this->data['sport_list']=$this->App_tv_model->AllSport();
			$this->data['fixture_list_count']=$this->App_tv_model->TotalFixtureCount($user_ref,$this->data['sport_id'],$this->data['date_from'],$this->data['date_end'],$this->data['search_text']);
			...
			$html = $this->load->view('app/download_schedule', $this->data, true);
	...
	{ 
		...
		//	print_r($ar);
			...
			$pdf->Output(APPPATH."pdf/".$pdfFilePath, "F");
				...
				$headers = "From: Sportshub365<info@sportshub365.com>" . $eol;
