<div class="events-inner" id="admin_events_inner">
       ...
       <div id="inline<?php echo $list['id'];?>" style="width:400px;display: none;">
		...
		<p> 
            ...
            <span class="form-row-popup">
            <input type="hidden" name="channel_display_id_<?php echo $list['id'];?>" value="<?php echo $list['channel_display_id'];?>" />
            </span>
             <span class="form-row-popup">
 ...
 if($total_records >= $ppr)
 {
  ...
  if($pagination['previous_page'] > 0)
  {
   ...
   <a href="javascript:void(0)"
    onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $pagination['previous_page'];?>','<?php echo $ppr;?>','<?php echo $search_channel;?>','<?php echo $sport_id;?>')"
  ...
  {
   ?>
   <a href="javascript:void(0)" onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $pagination['next_page'];?>','<?php echo $ppr;?>',<?php echo $search_channel;?>','<?php echo $sport_id;?>')" class="nextArrow3"><img style="margin-top:-6px;" src="<?php echo base_url();?>images/leftactive.jpg" alt="" /></a>
   <?php
  }
  else
  {
  ...
  ?>
   ...
   <ul>
   <?php if($start >= 2){?>
 <li><a href="javascript:void(0)"  onclick="SearchResultChannel('<?php echo base_url();?>admin/','1','<?php echo $ppr;?>','<?php echo $search_channel;?>','<?php echo $sport_id;?>')" <?php if($cp == $start){?> class="active" <?php } ?>> &lt;&lt; </a></li>
 <?php }?>
    <?php if(($start >= 10) ){
		$prevset = $start-7;
		if($prevset < 1)$prevset =1;
		?>
 <li><a href="javascript:void(0)"  onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $prevset;?>','<?php echo $ppr;?>','<?php echo $search_channel;?>','')" <?php if($cp == $start){?> class="active" <?php } ?>> &lt; </a></li>
 <?php }?>
  <?php 
      /*** declaring the page numbering based on current page Edited by Bagyaraj on Jul 23, 2015*///

 for($start;$start<=$endpage;$start++){ ?>
       <li><a href="javascript:void(0)"  onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $start;?>','<?php echo $ppr;?>','<?php echo $search_channel;?>','<?php echo $sport_id;?>')" <?php if($cp == $start){?> class="active" <?php } ?>><?php echo $start;?></a></li>
...
<?php }  
 ...
 ?>
    <?php if($pagination['total_page'] - $endpage >= 10){
		...
		?>
 <li><a href="javascript:void(0)"  onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $nextset;?>','<?php echo $ppr;?>','<?php echo $search_channel;?>','<?php echo $sport_id;?>')" <?php if($cp == $start){?> class="active" <?php } ?>> &gt; </a></li>
 <?php }?>

    <?php if($endpage < ($pagination['total_page'])){?>
 <li><a href="javascript:void(0)"  onclick="SearchResultChannel('<?php echo base_url();?>admin/','<?php echo $pagination['total_page'];?>','<?php echo $ppr;?>','<?php echo $search_channel;?>','<?php echo $sport_id;?>')" <?php if($cp == $start){?> class="active" <?php } ?>> &gt;&gt; </a></li>
 <?php }?>
