            ...
            <div class="events">
                   ...
                   <div class="event-form">
                        ...
                        <span class="event-form-box">
                        	<?php echo form_input($attribute_search['search_provider']);?>
   ...
   <script>
		...
		if(deleteid > 0) { 
			...
			confirm: function(){
				$.ajax
				({ 
					url: '<?php echo base_url();?>admin/delete_provider',
					...
					success: function(result) {
						...
						//if(result){
							...
							window.location = '<?php echo base_url();?>admin/providers';
						//}
						}
				});
				},
...
function update_provider_name(provider_id){
	var p_name = $('#provider_display_name'+provider_id).val();
	if(p_name !=''){
		$('#provider_display_name'+provider_id).removeClass('input_error');
		$.ajax({ 
		  url: '<?php echo base_url();?>admin/update_provider_name',
		  data: {id: provider_id, provider_name:p_name,type:'update_p_name'},
		  type: 'post',
		  success: function(result) {
			  	if(result == 2){
					$('#alreadyexists'+provider_id).html('Provider is already exists')
				}
				else if(result == 1){
					$('#provider_text_display_'+provider_id).html(p_name);
					parent.$.fancybox.close()
				}
				else if(result == 0){
					$('#alreadyexists'+provider_id).html('There is some prooblem while create')
				}
			}
		});
		...
		$('#provider_display_name'+provider_id).addClass('input_error');
	}
}
 ...
 $(document).ready(function() {
	$("#search_provider").keypress(function(event) {
		if (event.which == 13) {
			event.preventDefault();
			//alert('');
			var searchtext = $('#search_provider').val();
			var path = $('#path').val();
			SearchResultProvider(path,'1','20',searchtext);
			return false;
		}
	});
});
