  ...
  <div class="wrapper">
    ...
    <div id="content">
       <div class="container">
              ...
              <!--<a href="javascript:void(0)" class="add-new-event" data-cta-target=".js-dialog1" id="popup2" title="Click here to Add" onclick="javascript:NewChannelForm('<?php echo base_url();?>admin/');">New Channel</a>-->
            ...
            <div class="events">
				   ...
				   <?php $this->load->view('admin/channel_display.php');?>
              </div><!-- close events -->
         </div><!-- close container -->
    </div><!-- close content -->
...
function update_channel_display_name(channel_id){
	var ch_dp_name = $('#channel_display_name'+channel_id).val();
	if(ch_dp_name !=''){
		$('#channel_display_name'+channel_id).removeClass('input_error');
		$.ajax({ 
		  url: '<?php echo base_url();?>admin/update_channel_display_name',
		  data: {id: channel_id, type:'update_ch_dp_name', ch_dp_name:ch_dp_name},
		  type: 'post',
		  success: function(result) {
				if(result == 1){
		...
		$('#channel_display_name'+channel_id).addClass('input_error');
	}
}
</script>
 ...
 $(document).ready(function() {
	$("#search_channel").keypress(function(event) {
		if (event.which == 13) {
			event.preventDefault();
			//alert('');
			var searchtext = $('#search_channel').val();
			var path = $('#path').val();
			SearchResultChannel(path,'1','20',searchtext,'');
			return false;
		}
	});
});
