...
<?php  if(($provider_channels[0]['provider_id']!=10)) { ?>
	 <ul data-provider="" class="select_all">
            <li><span class="checkbox-box2"><div class="custom-checkbox"><input type="checkbox" <?php if(count($selected_channel_ids) == $total_records || count($selected_channel_ids)==0 ) { ?> checked="checked" <?php } ?> value="<?php ?>" id="select_all"><label for="select_all"></label></div></span>Select All</li>
    </ul>
 <?php } else { ?>
<ul data-provider="" class="select_all">
...
<?php	
 ...
 if($total_records >= $ppr)
 {
  ...
  {
   ?>
   <a href="javascript:void(0)" onclick="SearchProviderChannelResult('<?php echo base_url();?>establishment/','<?php echo $pagination['next_page'];?>','<?php echo $ppr;?>','<?php echo $search_text;?>','<?php echo $provider_id;?>')" class="nextArrow3"><img style="margin-top:-6px;" src="<?php echo base_url();?>images/leftactive.jpg" alt="" /></a>
   <?php
  }
  else
  {
 ...
 <?php }?>
    <?php if(($start >= 10) ){
		...
		?>
 <li><a href="javascript:void(0)"  onclick="SearchProviderChannelResult('<?php echo base_url();?>establishment/','<?php echo $prevset;?>','<?php echo $ppr;?>','<?php echo $search_text;?>','')" <?php if($cp == $start){?> class="active" <?php } ?>> &lt; </a></li>
 <?php }?>
  <?php 
      /*** declaring the page numbering based on current page Edited by Bagyaraj on Jul 23, 2015*///

 for($start;$start<=$endpage;$start++){ ?>
       <li><a href="javascript:void(0)"  onclick="SearchProviderChannelResult('<?php echo base_url();?>establishment/','<?php echo $start;?>','<?php echo $ppr;?>','<?php echo $search_text;?>','<?php echo $provider_id;?>')" <?php if($cp == $start){?> class="active" <?php } ?>><?php echo $start;?></a></li>
...
<?php }  
 ...
 ?> 
<span class="tvprovider_title" id="tvprovider-success" style="display:none">Selected channel(s) saved successfully. </span>  
