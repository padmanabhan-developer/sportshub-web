...
// Run the script on DOM ready:
 ...
 function save_provider_channel() {
		var prochannid = Array();	
		$('input[name="provider_channel"]:checked').each(function(index, element) {
            prochannid.push($(this).attr('id'))
        });
		//alert(providerid)
		if(prochannid == '') {
			$('#tvprovider-error').show();
			return false;
		}
		...
		$.ajax({
			...
			type: "POST",
			data: {prochannid:prochannid},
			beforeSend: function(data){
				 //$('input').customInput();
				},
			success: function(data){
				$('#tvprovider-success').show();
