     ...
     <div id="content">
     	  <div class="sectionmain">
          	   <div class="container">
                    ...
                    <form name="filter_bars" id="filter_bar" method="get" >

                        <div class="col-sm-12 ">
                            <div class="locationinput">
                                <div class="inputnew">
                                <input class="basic-slide" id="setlocation" name="name" type="text" value="<?php echo $filteraddress?>" placeholder="Type your location/destination... " />
                                <label for="setlocation" ><i class="fa fa-map-marker" aria-hidden="true"></i><span class="loc"></span>  Bars near you</label>
                                <input type="hidden" id="lat" name="lat"  value="<?php echo $latitude?>" />
                                <input type="hidden" name="lon" id="lon" value="<?php echo $longitude?>" />
                                <input type="hidden" name="address" id="address" value="<?php echo $filteraddress?>" />
                                </div>
                            </div>
                        </div>
				...
				?>  
                        <div class="barlist col-xs-12 col-sm-4">
                            <div class="carbox">
                            	<a class="img-carbox" style="background:url(<?php echo base_url();?>images/bar-img-1.jpg);" href="<?php echo base_url();?>promotion/bar/<?php echo $profile['id']?>/<?php echo $gamedetails['fixture_id'];?>">
                                    <div class="overlay">
                                   
                                    <div class="title4" ><p><?php echo $profile['title']?></p></div>
                                    </div>
                           		</a>
                            <div class="carbox-content">
                                <h4 class="carbox-title">
                                    <a class="bar_title"  href="<?php echo base_url();?>promotion/bar/<?php echo $profile['id']?>/<?php echo $gamedetails['fixture_id'];?>" >
                                    <?php if(trim($profile['address']) !='') echo $profile['address'];
                                    else echo 'Address currently not available'?>
                                    <br />
                                   <!-- <?php //echo $profile['zip']?> <?php //echo round($profile['distance'],2);?>-->
                                    </a>
                            ...
                            <div class="btn"><a href="<?php echo base_url();?>promotion/bar/<?php echo $profile['id']?>/<?php echo $gamedetails['fixture_id'];?>" class="detailbtn">Details ></a></div>
          ...
          <div class="section5">
                    ...
                    <span class="next2"><a href="<?php echo base_url();?>sportsfans">SPORTSLOVERS</a></span>
          </div><!--close section5-->
     </div><!--close content-->
...
<script type="text/javascript">  
    ...
    <script src="https://maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyChs_1wHfpiUXLq3OrtnIobdaquJi5ypSo&libraries=places"></script>
    <script type="text/javascript">
		google.maps.event.addDomListener(window,'load', initialize);
		function initialize(){
			var autocomplete = new google.maps.places.Autocomplete(document.getElementById('setlocation'));
			google.maps.event.addListener(autocomplete, 'place_changed',function(){
				...
				$('#lon').val(lng);
				$('#address').val($('#setlocation').val());
				$('#filter_bar').submit();
			});
		}
		/*$('#setlocation').keypress(function(e) {
			if (e.which == 13) {
			google.maps.event.addListener(autocomplete, 'place_changed',function(){
				alert('next');
				var place = autocomplete.getPlace();
