...
class Api extends REST_Controller {
	...
	public function matches_get()
	{
		...
		$lastMatch = $match_list_arr[ count($match_list_arr)-1 ];
		$sync_date = $lastMatch->date_time;
	...
	public function setfavorite_post()
	{
		...
		$sport_keys = $this->post('sport_keys');
		$league_keys = $this->post('competition_keys');
		$team_keys = $this->post('team_keys');
	...
	public function establishmentdetails_get()
	{
	...
	public function registerdevice_post()
	{
		...
		if( empty( $token ) ){
			...
			$values=array();
			$values['token']= $token;
			$values['user_key']= $this->post('user_key');
			$values['platform']= $this->post('platform');
