	...
	public function CheckUser($email,$password , $facebook_id )
	{
		...
		if( (!empty($email) and !empty($password)) || !empty($facebook_id) ){
			...
			if( valid_email($email) ){
				...
				$query = $this->db->query("SELECT user_id FROM user WHERE facebook_id = '".$facebook_id."' LIMIT 1", $this->db);
			}
			else{
			...
			// checking record exist in database 
				if($query->num_rows() > 0)
				{
					...
					if(!empty( $row->user_id )){
						...
						return array('status'=>'success','message'=>'Login Success','user_id'=>$row->user_id);
					}
					else
    ...
    /*public function Register($values)
    {
			...
			if(filter_var($values['email_id'], FILTER_VALIDATE_EMAIL)){
				...
				if($this->CheckIfUserExists( $values['email_id'] )==true){
					...
					return array('status'=>'failed','user_id'=> '', 'email' => $values['email_id'], 'firstname' => '', 'lastname'=>'', 'gender'=> '', 'phone' => '', 'country'=>'' , 'facebook_id' =>'', 'message'=>'User already exist with same email id');
...
<strong>Email :</strong> '.$email.'<br/><strong>Password :</strong> '.$password.'</p>';
    ...
    public function GetTeam($sportid, $search_text, $offset, $limit)
    {
	    ...
	    $sql="select * from team $cond $limits";
	    $query=$this->db->query($sql);
	    $rs=$query->result();
	 ...
	 public function GetTeamCount($sportid, $search_text)
    {
	    $cond = "WHERE deleted_on is NULL ";
