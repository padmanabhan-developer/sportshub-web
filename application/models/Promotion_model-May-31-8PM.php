  ...
  { 
   ...
   public function GetBarHappyHours($est_info_id)
	...
	public function GetAvailableLists($term){
		  ...
		  $sql_team="select * from team where team_name like '%$term%' order by team_name ";
		  $queryteam=$this->db->query($sql_team);
		  if($queryteam->num_rows()>0){
			 foreach($queryteam->result() as $team)
			 {
				$resultarray[]= array("category"=>'Team',"label"=>$team->team_name, "value"=>$team->team_id); 
			 }
		  }
	...
	$query = $this->db->query($sql);
		...
		$schedules = array( );
			...
			$schedules[$i] = array('id' => $row->fixture_id,'fixture_id' => $row->fixture_id,
					 ...
					 "date_time"=>$CetTime,"dateonly"=>$row->dateonly,"actualdate"=>$actualdate); 
			$i++;
			}
		}
	...
	public function GetAllMatches( )
    {
	    ...
	    $cond = "";
		$sync_date = date("Y-m-d H:i:s");
	    if(!empty($sync_date) && $sync_date != '' ) $cond.= " and  f.gmt_date_time >= NOW() ";
