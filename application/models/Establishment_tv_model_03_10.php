...
// end for date search 
  ...
  { 
	...
	$i = 0;
	  foreach($row as $channel)
		...
		$sql1 =  "SELECT pc.channel_name, pc.channel_name_dp FROM `establishment_provider_channel` as ec join `provider_channels` as pc on ec.`channel_id`=pc.`channel_id` WHERE pc.`channel_name`='$channel->channel_name' and ec.`est_ref`='$est_ref' UNION SELECT c.channel_name, c.channel_name as channel_name_dp FROM `establishment_provider_channel` as epc join `channel` as c on epc.`channel_id`=c.`channel_id` WHERE c.`channel_name`='$channel->channel_name' and epc.`est_ref`='$est_ref'";
 ...
 public function AllProvider()
  ...
  public function SelectedProvider($estref) {
	 $sp = array(); 
	$sql = "SELECT DISTINCT(epc.provider_id), p.provider_name, count(`channel_id`) as channel_count FROM `establishment_provider_channel` as epc join `provider` as p ON epc.provider_id = p.provider_id where epc.est_ref = '".$estref."' and status='1' group by `provider_id` order by p.provider_id asc ";
  ...
  public function SelectedProviderIds($estref) {
	 $sp = array(); 
	$sql = "SELECT DISTINCT(epc.provider_id), p.provider_name FROM `establishment_provider_channel` as epc join `provider` as p ON epc.provider_id = p.provider_id where epc.est_ref = '".$estref."' order by p.provider_id asc ";
	$query=$this->db->query($sql);
	$row=$query->result();
...
*/					 	$j++;	
		...
		if($query->num_rows()>0)
		{
			 ...
			 foreach($query->result() as $row)
			 {
			   $sql1 = "SELECT * FROM provider_channels WHERE channel_name = '$row->channel_name' ";
			   $query1 = $this->db->query($sql1);
				$row1 = $query1->result();
				//echo "<pre>";
				//print_r($row1); 
