...
// end for date search 
    ...
    t1.team_name as team1,t2.team_name as team2, c.competition_name from fixture f inner join team t1 on
     ...
     establishment_schedule es on f.fixture_id=es.fixture_ref inner join 
    competition c on f.rel_competition_id=c.competition_id  where es.establishment_ref='$est_ref_id' and f.gmt_date_time > CURDATE() 
      ...
      if($query->num_rows()>0)
       {
         ...
         foreach($query->result() as $row)
         {
		  ...
		  $channel_list = implode('<br>',$channels);
...
// end for date search 
 ...
 public function ConvertOneTimezoneToAnotherTimezone($time,$currentTimezone,$timezoneRequired)
  ...
  { 
	...
	//echo	$sql1 =  "SELECT pc.channel_name, pc.channel_name_dp FROM `establishment_provider_channel` as ec join `provider_channels` as pc on ec.`channel_id`=pc.`channel_id` WHERE pc.`channelid`='".mysql_real_escape_string($channel->rel_channel_id)."' and ec.`est_ref`='$est_ref' UNION SELECT c.channel_name, c.channel_name as channel_name_dp FROM `establishment_provider_channel` as epc join `channel` as c on epc.`channel_id`=c.`channel_id` WHERE c.`channel_name`='".mysql_real_escape_string($channel->channel_name)."' and epc.`est_ref`='$est_ref'";
	$sql1 =  "SELECT pc.channel_name_dp FROM `establishment_provider_channel` as ec join `provider_channels` as pc on ec.`channel_id`=pc.`channel_id` WHERE pc.`channelid`='".mysql_real_escape_string($channel->rel_channel_id)."' and ec.`est_ref`='$est_ref' ";
...
*/					 	$j++;	
 ...
 public function ProviderChannelPagenation($est_ref, $providerid, $provider, $limit='', $text){
	...
	$sql = "SELECT est_ref, channel_id, provider_id FROM `establishment_provider_channel` where est_ref = '".$estref."' $where order by channel_id asc ";	
	...
	public function SaveProviderChannel($providerid, $channelid, $check, $type, $est_ref){ 
		...
		if($type == 'single') {
			if($check == 'true') { 
				...
				$fixture_ids = $this->GetFixtureIdsFromProviderChannel($channelid); 
				//print_r($fixture_ids); die;
				if(count($fixture_ids)>0) {
					$fixture_id = implode(',', $fixture_ids);
					$sql_del="delete from establishment_schedule where establishment_ref='$est_ref' and fixture_ref in (".$fixture_id.")";
		...
		$channel_id = '';
		$sql1_d = "SELECT ep.channel_id FROM `establishment_provider_channel` as ep WHERE ep.est_ref= '$est_ref' and ep.provider_id='".$providerid."'";
		
		$query1_d = $this->db->query($sql1_d);
		if($query1_d->num_rows()>0) { 
			$channels = $query1_d->result();
			foreach($channels as $channel) {
				$channel_id = $channel->channel_id;
				$fixture_ids = $this->GetFixtureIdsFromProviderChannel($channel_id); 
				//print_r($fixture_ids); die;
				if(count($fixture_ids)>0) {
					$fixture_id = implode(',', $fixture_ids);
					$sql_del="delete from establishment_schedule where establishment_ref='$est_ref' and fixture_ref in (".$fixture_id.")";
	...
	public function GetFixtureIdsFromProviderChannel($channel_ids)
	  {
	   
	   $sql="select rel_fixture_id from fixture_channel_list where rel_channel_id = (select channelid from provider_channels where channel_id = '".$channel_ids."' ) ";
	   //echo $sql; die; 
	   $query=$this->db->query($sql);
	   if($query->num_rows()>0)
	   {
