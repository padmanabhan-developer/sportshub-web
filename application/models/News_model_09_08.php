...
return $news_data;
	...
	public function InsertNewsFeed($records)
	{
		...
		if(count($ids)==0)
		{
			$this->db->insert('news_feeds',$records);
			$resp= "inserted successfully";
		}
		else
		{ 
			$this->db->where('link',$records['link']);
			$this->db->update('news_feeds',$records);
	...
	{
		$this->db->select('id');
		$query = $this->db->get_where('news_feeds', array('link' => $link,'title'=>$title));
		//echo "<pre>";
