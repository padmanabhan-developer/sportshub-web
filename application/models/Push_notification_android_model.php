  ...
  public function GetEstablishmentNotifications($devices) {
			...
			//print_r($est_event); die;
				foreach($est_event as $event) {
					...
					if(count($token)>0) {
						//print_r($token); die;
						echo $this->push_gcm($token, $message, $array);	
						echo "<br>";	
					}
				}
			}
	    ...
	    if($query->num_rows() > 0) {
	    	...
	    	foreach( $result as $row ){
				...
				if( strtotime($diff_hours_24) == strtotime($matchhour) ) { //echo 'test';
					...
					$message1 = $row['team1'].' vs '.$row['team2'].' 24hrs to kick off! Click here to find nearest venue.';  			  
					$notify_arr = array();
					$notify_arr['message'] = $message;
					$notify_arr['user_key'] = $device['user_key'];
					$notify_arr['token'] = $device['token'];
				...
				// 2 hours before notification
					...
					$message2 = $row['team1'].' vs '.$row['team2'].' 2hrs to kick off! Click here to find nearest venue.';  		
					...
					$array2['name'] = "Team";
					$array2['id'] = $row['id'];
					$array2['matchtime'] = $matchhour;
					$token2[] = $device['token'];
					continue;
				}
	    	}
			if(count($token1)>0) {
				echo $this->push_gcm($token1, $message1, $array1);
				echo "<br>";
			}
			if(count($token2)>0) {
				echo $this->push_gcm($token2, $message2, $array2);
				echo "<br>";
			}
	    }
	    else  $rs = array();
