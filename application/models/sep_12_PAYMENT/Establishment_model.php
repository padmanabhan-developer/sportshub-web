 ...
 public function UpdateProfileInfo($arr,$email)
  ...
  public function SelectedChannels($estref) {
	...
	foreach($row as $val) {
		...
		$chid = $val->channelid;
		if($val->channelid!='') {
			$sql1 = "SELECT * FROM channel WHERE channel_id = '".$chid."' ";
  ...
  { 
    ...
    // end for date search 
     $sql="select f.fixture_id,c.rel_sport_id,date_format(f.gmt_date_time,'%d-%m-%Y') as gmt_date_time , f.gmt_date_time as local_time_form, f.rel_competition_id, f.timezone_date, f.timezone_time, 
      t1.team_name as team1,t2.team_name as team2, c.competition_name from fixture f inner join team t1 on
  ...
  public function GetProfileDetail($id)
  {
   ...
   if($query->num_rows()>0)
   {
    ...
    foreach($query->result() as $row)
    {
	  ...
	  if(!empty($row->city)) $sp['city']=$row->city;else $sp['city']='';
    if(!empty($row->zip)) $sp['zip']=$row->zip; else  $sp['zip']='';
  ...
  { 
    ...
    if($query_1->num_rows()>0){
     ...
     if(($row->subscribed !=1) && ($row->datediffval > 30)){
      ...
      $free_days_remaining = ($row->datediffval <=30) ? 30 -$row->datediffval: 0; 
      $subscription['free_days'] = $row->datediffval;
      $subscription['free_days_left'] = $free_days_remaining;
...
public function InsertPayment($data){
    ...
    //print_r($data);
    $data_0 = array('subscribed'=>'1','customer_id'=>$data['customer_id'],'subscription_start'=>$data['substarts'],'subscription_end'=>$data['subends'],'subscription_plan'=>$data['plan'],'subscription_status'=>$data['status']);
    $this->db->where('id',$data['est_ref']);
    $update_inf = $this->db->update('establishment_info',$data_0);
   // print_r($update_inf);
    //if($update_inf) echo 'updated';else 'no updated';
    /*echo $sql_update = "UPDATE establishment_info SET subscribed='1', customer_id='".$data['customer_id']."',subscription_start='".$data['substarts']."',  subscription_end='".$data['subends']."', subscription_plan='".$data['plan']."', subscription_status='".$data['status']."' WHERE id='".$data['est_ref']."'";
    $query_0 = $this->db->query($sql_update); */

    $data_1 = array('est_ref'=>$data['est_ref'],'customer_id'=>$data['customer_id'],'subscription_id'=>$data['subscription_id'],'subscription_start'=>$data['substarts'],'subscription_end'=>$data['subends'],'subscription_plan'=>$data['plan'],'subscription_status'=>$data['status'],'subscription_amount'=>$data['amount'],'card_id'=>$data['card_id'],'card_type'=>$data['card_object'],'card_number'=>$data['card_number'],'card_brand'=>$data['card_brand'],'card_funding'=>$data['card_funding'],'card_country'=>$data['card_country']);
    $insertinf = $this->db->insert('subscriptions', $data_1);
    //print_r($insertinf);
   // if($insertinf) echo 'Inserted'; else echo 'not inserted';
    //print_r($query); 
