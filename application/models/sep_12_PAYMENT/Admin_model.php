...
public function AdminLoginFormAttribute($values=array())
  ...
  /* $attribute['date_disable']=array('name'=> 'date_disable','id'=> 'datepicker-example12','value' => trim($values['date_end']),'class'=>"date-input" , 'onfocus'=>"if (this.value == 'Date End') {this.value = '';}" ,'onblur'=>"if (this.value == '') {this.value = 'Date End';}");*/
     ...
     $attribute['submit']=array('type' => 'button',  'name' => 'form_submit','class'=>'search-button','value'=>'',
      'onclick'=>$fun,'id'=>'search-button-schedule1');
     return $attribute;
  }
  ...
  public function SearchFormChannelAttribute($values=array())
  {
     if(count($values) == 0)
     {
      $values=array('date_from'=>'Date From','date_end'=>'Date End','search_channel'=>'Search');
     }
     $fun="SearchResultChannel(path.value,'1','20',search_channel.value,'');";
     $attribute['form']=array('id'=>'search_frm2','name'=>'search_frm2');

     $attribute['search_channel']=array('name'=> 'search_channel','id'=> 'search_channel','value' => trim($values['search_channel']),'class'=>"date-input" , 'onfocus'=>"if (this.value == 'Search') {this.value = '';}" ,'onblur'=>"if (this.value == '') {this.value = 'Search';}");
     
     $attribute['submit']=array('type' => 'button',  'name' => 'form_submit','class'=>'search-button','value'=>'',
      'onclick'=>$fun);
     return $attribute;
  }
  public function SearchFormProviderAttribute($values=array())
  {
     if(count($values) == 0)
     {
      $values=array('date_from'=>'Date From','date_end'=>'Date End','search_provider'=>'Search');
     }
     $fun="SearchResultProvider(path.value,'1','20',search_provider.value,'');";
     $attribute['form']=array('id'=>'search_frm2','name'=>'search_frm2');

     $attribute['search_provider']=array('name'=> 'search_provider','id'=> 'search_provider','value' => trim($values['search_provider']),'class'=>"date-input" , 'onfocus'=>"if (this.value == 'Search') {this.value = '';}" ,'onblur'=>"if (this.value == '') {this.value = 'Search';}");
     
     $attribute['submit']=array('type' => 'button',  'name' => 'form_submit','class'=>'search-button','value'=>'',
      'onclick'=>$fun);
     return $attribute;
  }
  public function SearchFormProviderChannelAttribute($values=array())
  {
     if(count($values) == 0)
     {
      $values=array('date_from'=>'Date From','date_end'=>'Date End','search_text_provider'=>'Search');
     }
     $fun="SearchResultProviderChannel(path.value,'1','20',search_text_provider.value,'');";
     $attribute['form']=array('id'=>'search_frm2','name'=>'search_frm2');

     $attribute['search_text_provider']=array('name'=> 'search_text_provider','id'=> 'search_text_provider','value' => trim($values['search_text_provider']),'class'=>"date-input" , 'onfocus'=>"if (this.value == 'Search') {this.value = '';}" ,'onblur'=>"if (this.value == '') {this.value = 'Search';}");
  ...
  public function AddChannelAttribute($values=array())
  {
    if(count($values) == 0)
    {
    $values=array('channel_name'=>'');
    }
    $attribute['form']=array('id'=>'channel_frm','name'=>'channel_frm', 'onSubmit'=>'return ValidateChannelForm();');
    $attribute['channel_name']=array('name'=> 'channel_name','id'=> 'channel_name','value' => trim($values['channel_name']),'class'=>"popup-input" , 'placeholder'=>"Channel Name:");
    $attribute['submit']=array('type' => 'submit',  'name' => 'form_submit','class'=>'popup-button','value'=>'Add Channel');
    return $attribute;
  }
  public function AddProviderAttribute($values=array())
  {
    if(count($values) == 0)
    {
    $values=array('channel_name'=>'');
...
public function viewpaymentinfo($esid,$cusid){ 
$sql=  "select s.*,ei.title from subscriptions s inner join establishment_info ei on s.est_ref = ei.id where s.est_ref='".$esid."' and s.customer_id ='".$cusid."' "; 
 ...
 return $result;
  ...
  public function paymenthistory($limit =''){ 
$sql=  "select s.*,ei.title from subscriptions s inner join establishment_info ei on s.est_ref = ei.id order by s.created_on desc $limit"; 
  $query = $this->db->query($sql);
    $result = $query->result();
    
 return $result;
  }
public function Generateinvoice($esid,$subsid){

 $sql = "select ei.*,s.*,eu.email from establishment_info ei inner join subscriptions s on ei.id= s.est_ref and s.sub_incr_id = '".$subsid."' inner join establishment_user eu on ei.est_user_ref = eu.id where ei.id = '".$esid."'";
...
public function GetInvoiceDetails($esid,$subsid){

$sql = "select ei.*,s.*,eu.email from establishment_info ei inner join subscriptions s on ei.id= s.est_ref and s.subscription_id = '".$subsid."' inner join establishment_user eu on ei.est_user_ref = eu.id where ei.id = '".$esid."'";
...
public function updateinvoicestatus($esid,$subsid, $invoice)
  {
    $sql="update subscriptions set invoice_status = '1', invoice_link='".$invoice."' where subscription_id = '".$subsid."' and est_ref = '" .$esid."'";
      $query=$this->db->query($sql);
...
// end for date search 
  ...
  public function EstablishmentAccountList($from_date='',$to_date='',$search_text='',$subscribed='',$limit='')
  { 
...
// end for date search 
    ...
    if($subscribed== '1')    {
      $where.=" and ei.subscribed ='1'";

    }
     if($subscribed== '0')    {
      $where.=" and ei.subscribed ='0'";
...
// end for date search 
       $sql="select ei.est_user_ref,ei.title,ei.id,ei.country,ei.address,ei.customer_id,ei.status,ei.totallikes,ei.subscribed,eu.first_name,eu.email from establishment_info as ei inner join
...
//die;
      ...
      $user[$i]['subscribed']=$row->subscribed;
      $user[$i]['customer_id']=$row->customer_id;
 ...
 public function DeleteEstablishmentInfo($id)
  ...
  ///redirect('admin/esatblishment');
    ...
    { 
     $est = $query_1->result()[0];
	 $sql_rem = "DELETE from establishment_info where id= '".$id."'  ";
     $query_rem = $this->db->query($sql_rem);
	  $sql_rem1 = "DELETE from establishment_user where id= '".$est->est_user_ref."'  ";
     $query_rem1 = $this->db->query($sql_rem1);
	  /*$data_ar['title'] = '';
  	  $data_ar['deleted_on'] = date('y-m-d:h:i:s');
      $this->db->where('id',$id);
 ...
 public function DeleteRating($id)
  ...
  public function Upgradedonwgradeaccount($id)
  {

    $sql = "UPDATE establishment_info
SET subscribed = '0', subscription_status = 'deactivated', modified_on = NOW()
WHERE id ='".$id."'";
$query = $this->db->query($sql);
return true;
  }
...
public function GetProfileFullDetail($estinfo)
  {
    $sql_1 ="select * from establishment_info ei.id= '$estinfo'";
...
//exit;*/
  ...
  public function ChannelList($search_text='',$limit='')
  { 
    $where="";
    if(!empty($search_text) && $search_text!='Search')
	{
      $where.=" and (c.channel_name like '%$search_text%')";
  	}
	$sql="select c.*,cd.channel_display_id, cd.channel_display_name from channel as c left join channel_display as cd on c.channel_id = cd.channel_id where 1=1  $where order by c.channel_name asc $limit ";
	$query=$this->db->query($sql);
	$channel=array();
	if($query->num_rows()>0)
	{
	 ...
	 foreach($query->result() as $row)
	 {
	   $channel[$i]['id']=$row->channel_id;
	  $channel[$i]['channel_name']=$row->channel_name;
	  $channel[$i]['channel_display_name']=$row->channel_display_name;
	  $channel[$i]['channel_display_id']=$row->channel_display_id;
	 // $user[$i]['account']=$row->account;
	  $i++;
	 }
	}
  ...
  public function ChannelListCount($search_text='',$limit='')
  { 
    $where="";
     
    if(!empty($search_text) && $search_text!='Search')
	{
      $where.=" and (channel_name like '%$search_text%')";
  	}
    
	$sql="select * from channel where 1=1  $where order by channel_name asc $limit ";
	$query=$this->db->query($sql);
	$channel=array();
	return $query->num_rows();
  }
  public function ProviderList($search_text='',$limit='')
  { 
    $where="";
    if(!empty($search_text) && $search_text!='Search')
	{
      $where.=" and (provider_name like '%$search_text%')";
  	}
 	$sql="select * from provider where 1=1  $where order by provider_name asc $limit ";
	$query=$this->db->query($sql);
	$channel=array();
	if($query->num_rows()>0)
	{
	 ...
	 foreach($query->result() as $row)
	 {
	   $channel[$i]['id']=$row->provider_id;
	  $channel[$i]['provider_name']=$row->provider_name;
	 // $user[$i]['account']=$row->account;
	  $i++;
	 }
	}
  ...
  public function ProviderListCount($search_text='',$limit='')
  { 
    $where="";
  ...
  public function ChannelProviderList($search_text='', $provider='',$limit='')
  { 
    $where="";
    if(!empty($search_text) && $search_text!='Search')
	{
	 ...
	 $pre_sel_qry = "select pc.channelid from channel c join provider_channels pc on (c.channel_id = pc.channelid and pc.provider_id='$provider' and pc.channel_id !=0) where 1=1  $where order by pc.channelid desc";
    $pre_sel=$this->db->query($pre_sel_qry);
    $pre_sel_res =  $pre_sel->result_array();
	$union_qry = "";
	$selected_channels_ar =array();
  	foreach($pre_sel_res as $pre_sel_r){
		$selected_channels_ar[] = $pre_sel_r['channelid'];
	}
	$selecte_channels ='';
	if(count($selected_channels_ar) > 0){
		$selecte_channels = implode(',',$selected_channels_ar);
		$union_qry = " and c.channel_id NOT IN($selecte_channels) ";
	}
	  $sql=" (select c.*,pc.channelid, pc.channel_name_dp from channel c join provider_channels pc on (c.channel_id = pc.channelid and pc.provider_id='$provider' and pc.channel_id !=0) where 1=1 $where order by c.channel_name asc ) UNION ALL (select c.*,cd.channel_id as channelid, cd.channel_display_name as channel_name_dp from channel c , channel_display cd where cd.channel_id = c.channel_id $union_qry  $where order by c.channel_name asc ) $limit ";

	$query=$this->db->query($sql);
	$channel=array();

    $sql_1="select channelid from  provider_channels where provider_id='$provider' and channelid !=0";
    $query1=$this->db->query($sql_1);
    $channel_id_res =  $query1->result_array();
	$channelid_ar =array();
  	foreach($channel_id_res as $channel_id){
		$channelid_ar[] = $channel_id['channelid'];
	}
	//print_r($channelid_ar);
  	if($query->num_rows()>0)
	{
	 $i=0;
	 foreach($query->result() as $row)
	 {
	   $selected = (in_array($row->channel_id,$channelid_ar)) ? 1 : 0;
	   $channel[$i]['id']=$row->channel_id;
	   $channel[$i]['channel_name']=$row->channel_name;
	   $channel[$i]['pc_channelid']=$row->channelid;
	   $channel[$i]['channel_name_dp']=$row->channel_name_dp;
	   $channel[$i]['selected']=$selected;
	 // $user[$i]['account']=$row->account;
	  $i++;
	 }
	}
  ...
  public function ChannelProviderListCount($search_text='', $provider='')
  { 
    $where="";
    if(!empty($search_text) && $search_text!='Search')
	{
	...
	if(count($selected_channels_ar) > 0){
	 ...
	 $sql=" (select c.* from channel c join provider_channels pc on (c.channel_id = pc.channelid and pc.provider_id='$provider' and pc.channel_id !=0) where 1=1 $where order by c.channel_name asc )  UNION ALL (select c.* from channel c where 1=1 $union_qry $where order by c.channel_name asc ) ";
	$query=$this->db->query($sql);
	$channel=array();
	return $query->num_rows();
  }
 public function GetProviderDetail($provider_id)
  {
    $sql_1 ="select * from provider where provider_id = '$provider_id'";
   ...
   if($query_1->num_rows()>0)
   {
    ...
    foreach($query_1->result() as $row)
    {
	  if(!empty($row->provider_id)) $sp['provider_id']=$row->provider_id;
      if(!empty($row->provider_name)) $sp['provider_name']=$row->provider_name;
	 }
    return $sp;
   }
  }
  public function UpdateChennelDisplayName($chid='',$ch_dp_name='')
  { 
    
     
    if(!empty($chid) && !empty($ch_dp_name))
	{
	 	$sql_check="select * from channel_display where channel_id = '".$chid."'";
		$query_check=$this->db->query($sql_check);
		 if($query_check->num_rows() > 0)
		 	{
				$sql="update channel_display set channel_display_name ='".$ch_dp_name."' where channel_id='".$chid."'";
				$sql1="update provider_channels set channel_name_dp ='".$ch_dp_name."' where channelid='".$chid."'";
				$query1=$this->db->query($sql1);
			}
		 else
		 	{
				$sql="INSERT INTO `channel_display` (`channel_display_name`, `channel_id`) values ('".$ch_dp_name."','".$chid."')";
			}
			$query=$this->db->query($sql);
		if($query) return 1;
		else return 0;
  ...
  public function UpdateProviderChennelInfo($provider,$channel, $mode)
  { 
    //echo $channel."--".$provider."--".$mode;
	if(!empty($channel) && !empty($provider))
	{
	 	$sql_check="select * from provider_channels where provider_id = '".$provider."' and channelid ='".$channel."'";
		$query_check=$this->db->query($sql_check);
		if(($query_check->num_rows() > 0) && ($mode == 'deselect')){
			$res_pro_ch_id = $query_check->result()[0];
			$sql_remove_0="delete from establishment_provider_channel where provider_id = '".$provider."' and channel_id ='".$res_pro_ch_id->channel_id."'";
			$query_remove_0=$this->db->query($sql_remove_0);
			$sql_remove_01="delete from app_provider_channel where provider_id = '".$provider."' and channel_id ='".$res_pro_ch_id->channel_id."'";
			$query_remove_01=$this->db->query($sql_remove_01);
			
			$sql_remove="delete from provider_channels where provider_id = '".$provider."' and channelid ='".$channel."'";
			$query_remove=$this->db->query($sql_remove);
			return 'deleted';
		}
		else if(($query_check->num_rows() == 0) && ($mode == 'select')){
			
			 $sql_ch_sel="select c.*,cd.channel_display_id, cd.channel_display_name from channel as c left join channel_display as cd on c.channel_id = cd.channel_id where c.channel_id = '".$channel."' limit 1 ";
			$query_ch=$this->db->query($sql_ch_sel);
			$res_ch = $query_ch->result()[0];
			//print_r($res_ch);
			//die;
			 $sql_insert="INSERT INTO `provider_channels` (`channel_name`, `channel_name_dp`, `provider_id`, `channelid`) VALUES ( '".$res_ch->channel_name."', '".$res_ch->channel_display_name."', '".$provider."', '".$channel."')";
			$query_insert=$this->db->query($sql_insert);
			if($query_insert) return 'inserted';
			else return 'error';
		}
		else return 'error';
...
public function CreateNewProvider($prov_name){
	if(!empty($prov_name))
	{
		
	 	$sql_check="select * from provider where provider_name = '".$prov_name."'";
		$query_check=$this->db->query($sql_check);
		if($query_check->num_rows() > 0){
			 return 2;
		}
		else{
			$sql_insert="INSERT INTO `provider` (`provider_name`, `created_on`) VALUES ( '".$prov_name."', NOW())";
			$query_insert=$this->db->query($sql_insert);
			if($query_insert) return 1;
			else return 0;
...
public function UpdateProviderName($pid, $prov_name){
	if(!empty($prov_name))
	{
		
	 	$sql_check="select * from provider where provider_name = '".$prov_name."' and provider_id != '".$pid."'";
		$query_check=$this->db->query($sql_check);
		if($query_check->num_rows() > 0){
			 return 2;
		}
		else{
			$sql_insert="UPDATE `provider` set `provider_name` = '".$prov_name."' , `modified_on`= NOW() WHERE provider_id='".$pid."'";
			$query_insert=$this->db->query($sql_insert);
			if($query_insert) return 1;
			else return 0;
 ...
 public function DeleteProviderInfo($id)
  {
   
    $sql_1 ="select * from provider where provider_id='".$id."'";
    $query_1 =$this->db->query($sql_1);
    
    if($query_1->num_rows()>0)
    { 
      $sql_rem = " delete from provider where provider_id='".$id."'";
      $query_rem = $this->db->query($sql_rem);
      $sql_rem1 = " delete from provider_channels where provider_id='".$id."'";
      $query_rem1 = $this->db->query($sql_rem1);
      $sql_rem2 = " delete from establishment_provider_channel where provider_id='".$id."'";
      $query_rem2 = $this->db->query($sql_rem2);
      return true;
    }
    else
    {
...
public function GetSubscriptionDetails($est_info_id)
  { 

    $sql_1 =  "select *, DATEDIFF(CURDATE(), DATE_FORMAT(`created_on`, '%Y-%m-%d')) as datediffval from establishment_info where id='".$est_info_id."'";
    $query_1=$this->db->query($sql_1);
    $subscription = array();
    if($query_1->num_rows()>0){
     $row=$query_1->result()[0];
     $row->datediffval;
     if(($row->subscribed !=1) && ($row->datediffval > 30)){
        $subscription['free_status'] = 0;
     } else{
        $subscription['free_status'] = 1;
     }
      $subscription['subscription'] = $row->subscribed;
      $subscription['customer_id'] = $row->customer_id;
      $subscription['subscription_start'] = $row->subscription_start;
      $subscription['subscription_end'] = $row->subscription_end;
      $subscription['subscription_plan'] = $row->subscription_plan;
      $subscription['subscription_status'] = $row->subscription_status;
    }
    return $subscription;
  }
}
