...
public function AdminLoginFormAttribute($values=array())
      ...
      // checking email id not empty
        ...
        if($query->num_rows() > 0)
        {
          ...
          if($query1){
				...
				$content .= '<a href="http://sportshub365.com/app/login" target="_blank" style="text-decoration:none; color:#fff; display:inline-block; font-family:Arial, Helvetica, sans-serif; font-size:13px; text-transform:uppercase; width:308px; height:42px; line-height:42px; text-align:center; padding:0; margin:0 0 34px 0; background:#1c6cd9;-webkit-border-radius: 10px;
 ...
 public function UpdateProfileInfo($arr,$email)
  ...
  { 
    ...
    t1.team_name as team1,t2.team_name as team2,c.competition_name from fixture f inner join team t1 on f.rel_team_id_1=t1.team_id
  ...
  { 
    ...
    if($query->num_rows()>0)
    {
	  ...
	  $channels='';
	  $channel_list='';
		  $channels = $this->GetFixtureChannel($row->fixture_id,$user_ref_id);
		  if(count($channels) > 0)
  ...
  public function GetFixtureChannel($fixtureid,$user_ref_id)
  { 
	...
	$i = 0;
	  foreach($row as $channel)
		...
		$sql1 =  "SELECT pc.channel_name, pc.channel_name_dp FROM `app_provider_channel` as ec join `provider_channels` as pc on ec.`channel_id`=pc.`channel_id` WHERE pc.`channel_name`='".mysql_real_escape_string($channel->channel_name)."' and ec.`user_ref`='$user_ref_id' UNION SELECT c.channel_name, c.channel_name as channel_name_dp FROM `app_provider_channel` as epc join `channel` as c on epc.`channel_id`=c.`channel_id` WHERE c.`channel_name`='".mysql_real_escape_string($channel->channel_name)."' and epc.`user_ref`='$user_ref_id'";
