...
// end for date search 
     $sql="select f.fixture_id,aps.sport_id,date_format(f.gmt_date_time,'%d-%m-%Y') as gmt_date_time , time_format(f.timezone_time,'%H:%i' ) as local_time_form, t1.team_name as team1,t2.team_name as team2, c.competition_name from fixture f inner join team t1 on f.rel_team_id_1=t1.team_id inner join team t2 on f.rel_team_id_2=t2.team_id inner join competition c on f.rel_competition_id=c.competition_id inner join app_schedule aps on f.fixture_id = aps.fixture_ref where aps.user_ref='$user_ref' and f.gmt_date_time > CURDATE() $cond  $where ORDER BY f.gmt_date_time $limit";
	 //echo $sql; die;	
      $query=$this->db->query($sql);
  ...
  { 
	...
	$i = 0;
	  foreach($row as $channel)
		...
		$sql1 =  "SELECT pc.channel_name, pc.channel_name_dp FROM `app_provider_channel` as ac join `provider_channels` as pc on ac.`channel_id`=pc.`channel_id` WHERE pc.`channelid`='".mysql_real_escape_string($channel->rel_channel_id)."' and ac.`user_ref`='$user_ref'";
		...
		if($type == 'single') {
			if($check == 'true') { 
				...
				if(count($fixture_ids)>0) {
					...
					$sql_del="delete from app_schedule where user_ref='$est_ref' and fixture_ref in (".$fixture_id.")";
			...
			$channel_id = '';
			$sql1_d = "SELECT ap.channel_id FROM `app_provider_channel` as ap WHERE ap.user_ref= '$user_ref' and ap.provider_id='".$providerid."'";
			...
			if($query1_d->num_rows()>0) { 
				...
				foreach($channels as $channel) {
					...
					if(count($fixture_ids)>0) {
						$fixture_id = implode(',', $fixture_ids);
						$sql_del="delete from app_schedule where user_ref='$user_ref' and fixture_ref in (".$fixture_id.")";
						$query=$this->db->query($sql_del);
					}
				}
