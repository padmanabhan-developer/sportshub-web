...
Please find the below for your </p>';*/
		...
		if($query->num_rows()>0)
		{
			...
			foreach($query->result() as $row)
			{
				...
				if(!empty($row->zip)) $sp['zip']=$row->zip;
				 $sp['phone']=$row->phone;
				 $sp['contact_email']=$row->contact_email;
  ...
  { 
    //$rs= $this->db_query->FetchInformation(SPORT,"","1='1'");
    //echo $est_ref_id;
	$subscription = $this->GetSubscriptionDetails($est_ref_id);
    $offer=array();
   
	if($subscription['subscription'] == 0 && $subscription['free_status']==0) {
	}
	else{
   ...
   public function GetBarHappyHours($est_info_id)
	...
	public function getBarEvents($est_info_id)
  	{ 
		//$rs= $this->db_query->FetchInformation(SPORT,"","1='1'");
		//echo $est_info_id;
		$subscription = $this->GetSubscriptionDetails($est_info_id);
		$event=array();
   	...
   	public function GetBarSchedules( $est_ref, $sync_date='' , $offset=0 , $limit=0 )
    {
	    ...
	    // Check Bar subscription status to limit matches for current day only
	    $subscription = $this->GetSubscriptionDetails($est_ref);
	    //print_r($subscription);
	    if($subscription['subscription'] == 0 && $subscription['free_status']==0) {
	    	 $sync_date = gmdate("Y-m-d H:i:s", time());
	    	 $time_end = date("Y-m-d"). " 23:59:59";
			 $cond.= " and  f.gmt_date_time >= '$sync_date' AND f.gmt_date_time <= '$time_end'";
	    }else{
	    ...
	    if($query->num_rows()>0)
       {
         ...
         foreach($query->result() as $row)
         {
			...
			$today = date('d-m-Y');  
			if($subscription['subscription'] == 0 && $subscription['free_status']==0 && ($today != $actualdate)) {
				continue;
			}
			...
			if($querysp->num_rows()>0)
			  ...
			  $schedules[$i]['actualdate']=$actualdate;
	...
	public function GetBars( $fixture_id,$latitude,$longitude,$distance,$sync_date,$sortby,$offset,$limit)
    {
	    ...
	    if(!empty($fixture_id)) {
	    	$queryfix = "select local_date from fixture where fixture_id='".$fixture_id."' AND local_date = CURDATE()";
			$queryfixres = $this->db->query($queryfix);
			$today_match = ($queryfixres->num_rows()>0) ? 1 : 0;
			
	    }
	    /*if($today_match == 0){
	    	//$cond = " WHERE est.title != '' and est.deleted_on is NULL AND est.status='0' and ( est.subscribed = '0' AND (DATEDIFF(CURDATE(), DATE_FORMAT(est.created_on, '%Y-%m-%d')) <= 30) ) OR  (est.subscribed = '1' AND est.subscription_end >= CURDATE())";
	    	$cond = " WHERE est.title != '' and est.deleted_on is NULL AND est.status='0' and (est.subscribed = '1' AND est.subscription_end >= CURDATE())";
	    	//$cond = " WHERE est.title != '' and est.deleted_on is NULL AND est.status='0'";
	    }
	    else */
	    	$cond = " WHERE est.title != '' and est.deleted_on is NULL AND est.status='0'";
	    //echo $cond;
		...
		if(($latitude !='') && ($longitude!='')){
	   	...
	   	$sql="SELECT est.id,
				est.title, DATEDIFF(CURDATE(), DATE_FORMAT(est.created_on, '%Y-%m-%d')) as createdatediff, est.subscribed, est.subscription_end, 
	...
	/*$sql="SELECT est.id,
	    ...
	    if($query->num_rows()>0)
       {
         ...
         foreach($query->result() as $row)
         {
          	  //echo $row->id."--".$row->createdatediff; echo "<br>";
         	if($today_match == 0 && $row->subscribed  == 0 && $row->createdatediff > 30){
	...
	public function GetAvailableLists($term){
		  ...
		  if($querycomp->num_rows()>0){
			 ...
			 {
				$resultarray[]= array("category"=>'League',"labeloriginal"=>$comp->competition_name,"label"=>$comp->competition_name, "value"=>$comp->competition_id); 
			 }
		  }
		  ...
		  if($queryspoort->num_rows()>0){
			 ...
			 {
				$resultarray[]= array("category"=>'Sport',"labeloriginal"=>$sport->sport_name,"label"=>$sport->sport_name, "value"=>$sport->sport_id); 
			 }
		  }
		  ...
		  if($queryteam->num_rows()>0){
			 ...
			 {
				$resultarray[]= array("category"=>'Team',"labeloriginal"=>$team->team_name,"label"=>$team->team_name ." (".$team->sport_name.") ", "value"=>$team->team_id); 
			 }
		  }
	...
	$cond = "where exists (select 1 from fixture_channel_list where rel_fixture_id = f.fixture_id) ";
	...
	competition c on f.rel_competition_id=c.competition_id where exists (select 1 from fixture_channel_list where rel_fixture_id = f.fixture_id) AND f.gmt_date_time >= '$sync_date'
	...
	return $sp;
 	...
 	public function GetMatchesDetailsBySearch($searchtext)
	{ 
	...
	if(!empty($searchtext))
	{
        $cond.=" (t1.team_name like '%".$searchtext."%' or t2.team_name like '%".$searchtext."%')";
		
		//$sport_ids=str_replace("~","','",$sport_id);
		$cond .=" OR (c.competition_name LIKE ('%".$searchtext."%'))";
		$resultarray = array();
		$sql_sport="select * from sport where sport_name like '%$searchtext%' order by sport_name ";
		  $queryspoort=$this->db->query($sql_sport);
		  if($queryspoort->num_rows()>0){
			 foreach($queryspoort->result() as $sport)
			 {
				$resultarray[]= $sport->sport_id; 
			 }
		  }
		if(count($resultarray) > 0){
		$sport_ids=implode(',',$resultarray);
		$cond .= " OR (c.rel_sport_id in (".$sport_ids."))";
		}
	}
	else 
		{
	...
	$sql="select f.fixture_id,c.rel_sport_id, c.competition_name, f.gmt_date_time, date_format(f.gmt_date_time, '%Y-%m-%d') as dateonly , time_format(f.timezone_time,'%H:%i' ) as local_time_form, f.rel_competition_id, f.rel_team_id_1, f.rel_team_id_2, f.rel_timezone_id, f.timezone_date, f.timezone_time, t1.team_name as team1,t2.team_name as team2 from fixture f inner join team t1 on
	f.rel_team_id_1=t1.team_id inner join team t2 on f.rel_team_id_2=t2.team_id inner join 
	competition c on f.rel_competition_id=c.competition_id where exists (select 1 from fixture_channel_list where rel_fixture_id = f.fixture_id) AND f.gmt_date_time >= '$sync_date' AND ( $cond ) ORDER BY f.gmt_date_time $limit";
	//echo $sql; //die;
	$query=$this->db->query($sql);
	$row=$query->result();
		if($query->num_rows()>0)
	...
	public function GetAllMatches( )
    {
	    ...
	    $cond.= " Limit  250 ";
	  //$sql="select f.*,date_format(f.gmt_date_time, '%Y-%m-%d') as dateonly, c.competition_name  from fixture f inner join competition c ON f.rel_competition_id = c.competition_id inner join fixture_channel_list fcl ON f.fixture_id = fcl.rel_fixture_id WHERE 1=1  $cond";
   	  $sql="select f.*,date_format(f.gmt_date_time, '%Y-%m-%d') as dateonly, c.competition_name  from fixture f, competition c WHere exists (select 1 from fixture_channel_list where rel_fixture_id = f.fixture_id) and c.competition_id = f.rel_competition_id $cond";
  ...
  {   
    ...
    if($query_1->num_rows()>0){
     ...
     if(($row->subscribed !=1) && ($row->datediffval > 30)){
      ...
      $subscription['free_days'] = $row->datediffval;
