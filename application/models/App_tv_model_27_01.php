...
// end for date search 
  ...
  { 
	...
	$i = 0;
	  foreach($row as $channel)
		...
		$sql1 =  "SELECT pc.channel_name, pc.channel_name_dp FROM `app_provider_channel` as ac join `provider_channels` as pc on ac.`channel_id`=pc.`channel_id` WHERE pc.`channel_name`='$channel->channel_name' and ac.`user_ref`='$user_ref'";
 ...
 public function AllProvider()
  ...
  public function SelectedProvider($estref) {
	 $sp = array(); 
	$sql = "SELECT DISTINCT(apc.provider_id), p.provider_name, count(`channel_id`) as channel_count FROM `app_provider_channel` as apc join `provider` as p ON apc.provider_id = p.provider_id where apc.user_ref = '".$estref."' and status='1' group by `provider_id` order by p.provider_id asc ";
  ...
  public function SelectedProviderIds($userref) {
	 $sp = array(); 
	$sql = "SELECT DISTINCT(apc.provider_id), p.provider_name FROM `app_provider_channel` as apc join `provider` as p ON apc.provider_id = p.provider_id where apc.user_ref = '".$userref."' order by p.provider_id asc ";
	$query=$this->db->query($sql);
	$row=$query->result();
