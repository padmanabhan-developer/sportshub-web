...
class Table_model extends CI_Model
{
    ...
    public function GetDevices()
    {
        ...
        $sql="select * from devices_android";
        $query=$this->db->query($sql);
        $rs=$query->result();
    ...
    public function GetWeekConstantList()
    {
  	 ...
  	 public function InsertOrUpdateDevice($array) {
		...
		$rs = array();
		if($query->num_rows() > 0) { 
			$sql1 = "select * from devices_android where user_key='$user_key' ";
			...
			if($query1->num_rows() > 0) { 
				...
				$this->db->update('devices_android', $arraydata);
				$id = $result1[0]->id;	
			}
			else {
				//echo "<pre>";
				//print_r($array); die;
				$this->db->insert( 'devices_android', $array );
				$id = $this->db->insert_id();	
			}
			$sql2 = "select * from devices_android where id='$id' ";
