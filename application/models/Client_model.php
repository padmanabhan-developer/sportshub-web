 ...
 { 
  //echo "<br>".$list['action'];
    ...
    if ($flag==true)
    {
		...
		$this->db->where('channel_id', $list['channel_id']);
		$this->db->update('channel_display',$recdp);
		...
		$this->db->update('provider_channels',$recpc);
		...
		$recdp['channel_display_name']=$list['channel_name'];
		$recdp['created_on']=$currTime;
		$this->db->insert('channel_display',$recdp);
...
public function SetSyncDate($dat,$comment='')
{
 // $data['sync_date']=$dat;
