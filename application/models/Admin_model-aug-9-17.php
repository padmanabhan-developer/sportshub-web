...
//exit;*/
  ...
  { 
    ...
    if(!empty($search_text) && $search_text!='Search')
	{
	 ...
	 $sql=" (select c.*,pc.channelid, pc.channel_name_dp from channel c join provider_channels pc on (c.channel_id = pc.channelid and pc.provider_id='$provider' and pc.channel_id !=0) where 1=1  $where order by pc.channelid desc )  UNION ALL (select c.*,pc.channelid, pc.channel_name_dp from channel c left join provider_channels pc on (c.channel_id = pc.channelid and pc.provider_id ='XXXXX' and pc.provider_id !='$provider' and pc.channel_id !=0 ) where 1=1  $where order by pc.channelid desc ) $limit ";
