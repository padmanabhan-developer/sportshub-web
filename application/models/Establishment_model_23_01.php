...
public function AdminLoginFormAttribute($values=array())
 ...
 public function UpdateProfileInfo($arr,$email)
  ...
  public function SignupFormAttribute($values=array())
  {
    if(count($values) == 0)
    {
    $values=array('re_password'=>'','email'=>'Mail','password'=>'','title' => 'Title / name at Venue', 'resellercode' => 'Reseller Code');
    }
    $attribute['form']=array('id'=>'signup_frm','name'=>'signup_frm');

	$attribute['title']=array('name'=> 'title','id'=> 'title','value' => trim($values['title']),'class'=>"title-input" , 'onfocus'=>"if (this.value == 'Title / name at Venue') {this.value = '';}" ,'onblur'=>"if (this.value == '') {this.value = 'Title / name at Venue';}");
  ...
  { 
    ...
    if(!empty($search_text) && $search_text!='Search') $where.=" and (t1.team_name like '%$search_text%' or t2.team_name like '%$search_text%')";
    // end for date search 
     $sql="select f.fixture_id,c.rel_sport_id,date_format(f.gmt_date_time,'%d-%m-%Y') as gmt_date_time , f.gmt_date_time as local_time_form, f.rel_competition_id, 
