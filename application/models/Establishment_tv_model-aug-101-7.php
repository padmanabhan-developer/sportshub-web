	...
	$i = 0;
	  ...
	  //echo $sql1; die;
	   ...
	   if($query1->num_rows()>0)
	   {
		  ...
		  $result[$i] =	($row1[0]->channel_name_dp) ? $row1[0]->channel_name_dp : $channel->channel_name;
		   $i++;
	   }
	   else{
...
*/					 	$j++;	
 ...
 public function ProviderChannelPagenation($est_ref, $providerid, $provider, $limit='', $text){
	...
	$sql = "SELECT est_ref, channel_id, provider_id FROM `establishment_provider_channel` where est_ref = '".$estref."' order by channel_id asc ";	
