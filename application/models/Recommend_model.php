...
class Recommend_model extends CI_Model {
  ...
  public function SearchResellerAttribute($values=array())
  {
     ...
     $fun="SearchResultRecabar(path.value,'1','20',date_from.value,date_end.value,search_text.value);";
  ...
  public function GetRecabarData($resellerid='',$from_date,$to_date,$search_text, $limit='') {
	  
	  $cond = 'where 1=1';
	  if($resellerid!='') $cond.=" and id ='$resellerid'";
      ...
      if(!empty($search_text) && $search_text!='Search') $cond.=" and (barname like '%$search_text%')";
	  // if($status!='') { $cond.=" and status IN ('$status')"; }	
	  
	  $sql = "select * from establishment_recommendbar $cond order by created_on DESC $limit";
	 // echo $sql; die; 
	  ...
	  if($query->num_rows()>0)
       {
		...
		{
		  $sp[$i]['id'] = $row->id;
		  $sp[$i]['barname'] = $row->barname;
		  $sp[$i]['baraddress'] = $row->baraddress;
		  $sp[$i]['baremail'] = $row->baremail;
		  $sp[$i]['barphone'] = $row->barphone;
		  $sp[$i]['useremail'] = $row->useremail;
		  $sp[$i]['userphone'] = $row->userphone;
  ...
  public function TotalRecbar($id='',$from_date,$to_date,$search_text,$limit='') {
...
<strong>Reseller Code :</strong> '.$arr['resellercode'].'</p>';
  ...
  public function DeleteRecabar($id)  {
    $sql ="select * from establishment_recommendbar where id='$id'";
    $query =$this->db->query($sql);
    //echo $sql;  die;
    if($query->num_rows()>0)
    { 
        $this->db->where('id',$id);
		$this->db->delete('establishment_recommendbar');
		return true;
    }
    else
    {
